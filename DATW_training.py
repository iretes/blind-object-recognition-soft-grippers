...
def train_and_eval(
    ...
    print("Testing")
    y_pred = datw.predict(X_ref=X_train, y_ref=y_train, X_test=X_test, y_test=y_test)
    
    print("Saving model, training history and predictions")
    torch.save(datw.best_model, os.path.join(results_subdir, f"DATW.pkl"))
    with open(os.path.join(results_subdir, f"DATW_pretraining_history.pkl"), 'wb') as f:
        pickle.dump(pretraining_history, f)
    with open(os.path.join(results_subdir, f"DATW_contrastive_history.pkl"), 'wb') as f:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pre_training_num_epochs', type=int, default=10,
        help='Number of epochs for pre-training.'
    )
    parser.add_argument('--pre_training_iteration', type=int, default=100,
        help='Number of iterations for pre-training.'
    )
